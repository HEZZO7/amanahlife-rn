...
- ~~`app_11941c8fec_savings_tips` discrepancy~~ — **resolved 2026-07-23**. The real, deployed function lives in the **`AmanahLifeapp` repo** at `app/frontend/supabase/functions/app_11941c8fec_savings_tips/index.ts` — a separate implementation from the one this session (mistakenly) first built in `amanahlife-rn` (now deleted, see below). Per Huzaifa's decision, that web-repo function was updated in place (commit `80caf97`): swapped its DeepSeek call for Anthropic (`claude-haiku-4-5-20251001`, same model as `app_11941c8fec_ai_life_coach`, same `ANTHROPIC_API_KEY` secret — no second AI provider key to manage), added the same JWT auth + CORS scoping as `ai_life_coach` (previously fully open, no auth, wildcard CORS), and kept its existing 5-tip bilingual fallback untouched. Live-tested before the fix: the AI-generation path was silently dead (DeepSeek key likely missing/invalid) — every request, even with realistic distinct challenge data, returned the exact same static fallback string. Not yet re-verified live after this fix (needs a fresh probe once redeployed — this is a web-repo Edge Function, redeploy is automatic on push same as the frontend... **actually confirm this**, Edge Functions may need a separate `supabase functions deploy` unlike the frontend's Coolify auto-deploy).
- 🚨 **Major discovery while investigating the above: there are TWO separate `supabase/functions/` directories in the `AmanahLifeapp` repo** — `app/frontend/src/supabase/functions/` and a sibling `app/frontend/supabase/functions/` (note: no `src/`). They are NOT copies of each other for at least `lemonsqueezy_checkout`/`lemonsqueezy_webhook` — genuinely different implementations, confirmed via `diff`. Git history shows Huzaifa explicitly declared `app/frontend/supabase/functions/` **"true source of record"** in commit `0e312d5` (2026-07-04), synced from the live Atoms Dev environment — **two weeks before** this session's Phase 4 Lemon Squeezy security hardening (commit `4161aa6`), which only touched the `src/` copy. **This means the constant-time signature check, CORS scoping, tier-derived-from-variant-id fix, and open-redirect fix reported as done in Phase 4 may never have reached what's actually deployed.** Not yet fixed or even fully audited — needs Huzaifa's decision on how to proceed (same question as the savings_tips case: which copy is real, should `src/` be deleted entirely). The non-`src/` directory also has `app_11941c8fec_delete_account`, `exchange_rates`, `push_notify`, and `weekly_digest` — four functions never audited or even read this session, since `src/supabase/functions/` was the only path checked throughout (this second directory was tracked in git but missing from every local working-tree checkout this session, silently masked by a `git status` filter used to hide ~200 unrelated pre-existing deletions — never verified whether that filter was hiding something real until asked to look specifically). **Do not assume any Phase 4 web security fix took effect in production until this is resolved.**
