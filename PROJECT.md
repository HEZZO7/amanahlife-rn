...
- **Two Edge Functions are written but not yet deployed to Supabase**: `app_11941c8fec_ai_life_coach` and `app_11941c8fec_savings_tips` (both Anthropic-backed, same pattern, both need only the already-set `ANTHROPIC_API_KEY` secret — no new secret required). The *frontend* code calling them is live on both platforms; until these are deployed, the AI coach and daily savings tip will return an "unavailable" error in the app rather than a real response. Deploy via the Supabase dashboard (Edge Functions → Deploy a new function) — needs project-owner access, which this session doesn't have.
